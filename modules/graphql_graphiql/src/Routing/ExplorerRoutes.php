...
namespace Drupal\graphql_graphiql\Routing;
...
class ExplorerRoutes extends RouteSubscriberBase {
  ...
  protected function alterRoutes(RouteCollection $collection) {
    ...
    foreach ($this->schemaManager->getDefinitions() as $key => $definition) {
      $routes->add("graphql_graphiql.explorer.$key", new Route("{$definition['path']}/explorer", [
        'schema' => $key,
        '_controller' => '\Drupal\graphql_graphiql\Controller\ExplorerController::viewExplorer',
        '_title' => 'GraphiQL',
      ], [
